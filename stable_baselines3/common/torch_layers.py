...
class ImpalaCNN(BaseFeaturesExtractor):
    ...
    def forward(self, observations: th.Tensor) -> th.Tensor:
        # Note(Mandi): CoinRun Env already normzied to 0-1!
        return self.linear(self.cnn(observations))
    ...
    def __init__(self, observation_space: gym.spaces.Dict, cnn_output_dim: int = 256, cnn_arch: Union[str, None] = 'NatureCNN'):
        ...
        for key, subspace in observation_space.spaces.items():
            if is_image_space(subspace):
                if 'NatureCNN' in cnn_arch:
                    extractors[key] = NatureCNN(subspace, features_dim=cnn_output_dim)
                elif 'ImpalaCNN' in cnn_arch:
                    extractors[key] = ImpalaCNN(subspace, features_dim=cnn_output_dim)
                else:
                    raise NotImplementedError
