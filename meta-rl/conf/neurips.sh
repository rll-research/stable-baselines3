...
LOAD_RUN=ImpalaCNN-1e4levels-256Envs-2048Batch/seed1 
LOAD_STEP=1500
for LEVEL in 10000 10002 10007 10009 10015 
do
    RUN=FineTune-1e4-Unseen${LEVEL} 
    taskset -c $CPUS python train.py run_name=$RUN \
        load_run=$LOAD_RUN load_step=$LOAD_STEP \
        procgen.train.start_level=${LEVEL} procgen.train.num_levels=1 \
        learn.total_timesteps=1e6 procgen.eval.start_level=${LEVEL} procgen.eval.num_levels=1 \
...
do
    ...
    taskset -c $CPUS python train.py run_name=$RUN \
        ...
        procgen.train.start_level=${LEVEL} procgen.train.num_levels=1 \
        learn.total_timesteps=5e6 procgen.eval.start_level=${LEVEL} procgen.eval.num_levels=1 \
        learn.eval_freq=1 learn.log_interval=1  learn.n_eval_episodes=640 log_wb=False vb=3
done

LEVEL=10000
RUN=Scratch-Unseen${LEVEL} 
taskset -c $CPUS python train.py run_name=$RUN procgen.train.num_envs=64 \
    procgen.train.start_level=${LEVEL} procgen.train.num_levels=1 \
    learn.total_timesteps=5e6 procgen.eval.start_level=${LEVEL} procgen.eval.num_envs=50 procgen.eval.num_levels=1 \
